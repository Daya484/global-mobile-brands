...
def main():
    ...
    log.info("Loading BigQuery dimensions: %s.%s.*",dv-env, args.dataset)
    try:
        dim_calender = read_bq_table(spark, dv-env, args.dataset, "dim_date")
        dim_market   = read_bq_table(spark, dv-env, args.dataset, "dim_market")
        # .cache() keeps these DataFrames in memory after first use
        # so they don't get re-read from BQ if used in multiple joins
        dim_product  = read_bq_table(spark, dv-env, args.dataset, "product_v2").cache()
        dim_customer = read_bq_table(spark, dv-env, args.dataset, "customer").cache()
